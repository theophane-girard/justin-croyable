...
export interface StringTemplateOutletContext {
  $implicit: unknown;
  [key: string]: unknown;
}
...
export class StringTemplateOutletDirective<T = unknown> implements OnDestroy {
  private readonly viewContainer = inject(ViewContainerRef);
  private readonly templateRef = inject(TemplateRef<void>);

  private embeddedViewRef: EmbeddedViewRef<StringTemplateOutletContext> | null = null;
  private readonly context = {} as StringTemplateOutletContext;

  #isFirstChange = true;
  #lastOutletWasTemplate = false;
  #lastTemplateRef: TemplateRef<void> | null = null;
  #lastContext?: StringTemplateOutletContext;

  readonly stringTemplateOutletContext = input<StringTemplateOutletContext | undefined>(undefined, {
    alias: 'appStringTemplateOutletContext',
  });
  readonly stringTemplateOutlet = input.required<T | TemplateRef<void>>({ alias: 'appStringTemplateOutlet' });

  #hasContextShapeChanged(context: StringTemplateOutletContext | undefined): boolean {
    if (!context) {
      return false;
    }
  ...
  #shouldViewBeRecreated(
    ...
    stringTemplateOutletContext: StringTemplateOutletContext | undefined,
  ): boolean {
  ...
  readonly #viewEffect: EffectRef = effect(() => {
    const stringTemplateOutlet = this.stringTemplateOutlet();
    const stringTemplateOutletContext = this.stringTemplateOutletContext();
    ...
    if (recreateView) {
      this.#recreateView(
        stringTemplateOutlet as TemplateRef<StringTemplateOutletContext>,
  ...
  #recreateView(
    outlet: TemplateRef<StringTemplateOutletContext>,
    context: StringTemplateOutletContext | undefined,
  ): void {
  ...
  #updateContext(outlet: TemplateRef<void> | T, context: StringTemplateOutletContext | undefined): void {
