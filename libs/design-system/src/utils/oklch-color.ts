...
export function scaleOklch(color: OklchColor, lightness: number, chroma: number): OklchColor {
  return {
    lightness: Math.min(color.lightness * lightness, MAX_LIGHTNESS),
...
function toLinearSrgb({ lightness, chroma, hue }: OklchColor): readonly number[] {
  ...
  const greenAxis = chroma * Math.cos(radians);
  const blueAxis = chroma * Math.sin(radians);
  const long = (lightness + 0.3963377774 * greenAxis + 0.2158037573 * blueAxis) ** CUBED;
  const medium = (lightness - 0.1055613458 * greenAxis - 0.0638541728 * blueAxis) ** CUBED;
  const short = (lightness - 0.0894841775 * greenAxis - 1.291485548 * blueAxis) ** CUBED;
  return [
...
function toHexPair(channel: number): string {
  const clamped = Math.min(Math.max(channel, 0), MAX_LIGHTNESS);
  const encoded =
