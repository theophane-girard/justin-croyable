...
import { MENU_POSITIONS_MAP, type MenuPlacement } from './menu-positions';
...
export class MenuDirective implements OnInit, OnDestroy {
  private static readonly MENU_CONTENT_SELECTOR = '.cdk-overlay-pane [app-menu-content]';
  ...
  readonly menuTriggerFor = input.required<TemplateRef<void>>();
  readonly disabled = input<boolean, BooleanInput>(false, { transform: booleanAttribute });
  readonly trigger = input<MenuTrigger>('click');
  readonly hoverDelay = input<number>(100);
  readonly placement = input<MenuPlacement>('bottomLeft');

  private readonly menuPositions = computed(() => this.getPositionsByPlacement(this.placement()));
  ...
  private getPositionsByPlacement(placement: MenuPlacement): ConnectedPosition[] {
  ...
  private setupMenuContentListeners(): void {
    const menuContent = this.document.querySelector(MenuDirective.MENU_CONTENT_SELECTOR);
    if (!menuContent) {
      return;
    }
  ...
  private shouldKeepMenuOpen(relatedTarget: Element | null): boolean {
    ...
    const isMovingToMenu = relatedTarget.closest(MenuDirective.MENU_CONTENT_SELECTOR);
    const isMovingToOtherTrigger =
      relatedTarget.matches('[app-menu]') && !this.elementRef.nativeElement.contains(relatedTarget);
