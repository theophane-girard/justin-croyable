...
export const calendarDayButtonVariants = cva(
  ...
  // `cursor-not-allowed` : les classes sont fusionnées par `mergeClasses`
  // (twMerge), donc c'est bien la variante, appliquée en dernier, qui gagne.
  'p-0 font-normal flex cursor-pointer items-center justify-center whitespace-nowrap rounded-md ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 hover:bg-accent hover:text-accent-foreground size-full text-sm',
  {
    variants: {
      selected: {
