...
/**
 * Reference to a dialog opened via {@link DialogService}.
 *
 ...
 */
export class DialogRef<T = unknown, R = unknown, U = unknown> {
  /** Stack of currently open dialogs. The last entry is the topmost. */
  private static readonly stack: DialogRef[] = [];
  ...
  constructor(
    ...
    private readonly config: DialogOptions<T, U>,
    private readonly containerInstance: DialogComponent<T, U> | null,
    private readonly platformId: object,
  ) {
    this.animationDuration = config.duration ?? 100;
    ...
    DialogRef.stack.push(this as unknown as DialogRef);
  ...
  private dispose() {
    ...
    const idx = DialogRef.stack.indexOf(this as unknown as DialogRef);
    if (idx >= 0) DialogRef.stack.splice(idx, 1);
