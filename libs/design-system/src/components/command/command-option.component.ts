...
import type { CommandOptionGroupComponent } from './command-option-group.component';
...
@Component({
  ...
  template: `
    @if (isOptionVisible()) {
      ...
      >
        @if (icon()) {
          <ng-icon [name]="icon()!" class="mr-2 flex size-4! shrink-0 items-center justify-center" />
        }
  ...
  exportAs: 'commandOption',
})
export class CommandOptionComponent {
  private readonly elementRef = inject(ElementRef);
  private readonly parentCommandComponent = inject(CommandComponent, { optional: true });
  ...
  readonly parentCommand = input<CommandComponent | null>(null);
  readonly commandGroup = input<CommandOptionGroupComponent | null>(null);
