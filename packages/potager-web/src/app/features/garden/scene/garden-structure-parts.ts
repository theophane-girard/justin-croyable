...
export function buildFieldParts(
  ...
  const furrows = Array.from({ length: furrowCount }, (_, index): ScenePartDraft => {
    ...
    return {
      geometry: PART_GEOMETRY.box,
      args: [width * hashRange(index + 1, 0.94, 1), FURROW_HEIGHT, FURROW_WIDTH],
...
export function buildBedParts(colors: GardenSceneColors, plankColor: string): ScenePart[] {
...
export function buildMarkerParts(colors: GardenSceneColors): ScenePart[] {
